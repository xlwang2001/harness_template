...
The Linear tracker adapter also exposes explicit write APIs for workflow-owned mutations: `add_comment`, `transition_issue`, and `record_pull_request`. The orchestrator does not call these by default; normal dispatch remains scheduler/reader oriented, and generated project prompts can still ask the coding agent to make workflow-specific ticket updates. Use these APIs only from deliberate workflow code or operator tooling, with credentials scoped to the target Linear project.
