...
When the runtime uses the Linear tracker, it can answer the optional `linear_graphql` client-side tool request. The tool reuses configured Linear credentials, accepts one GraphQL operation per call, and returns structured success or failure payloads that the agent can inspect without reading raw tokens. The current targeted generated Codex schema does not expose a stable startup tool advertisement field, so the runner intentionally does not send non-schema `client_tools`, `tools`, or `dynamicTools` fields. If a future schema adds a supported advertisement field, `make codex-schema-test` should flag that this decision needs a deliberate implementation update.

The Linear tracker adapter exposes explicit write APIs for workflow-owned mutations: `add_comment`, `transition_issue`, and `record_pull_request`. When `tracker.handoff_state` is set, the orchestrator calls `transition_issue` after a successful agent session and does not schedule a same-issue continuation retry. Leave `handoff_state` unset to keep completion handoff fully prompt/tool-driven. Use write APIs only from deliberate workflow code or operator tooling, with credentials scoped to the target Linear project.

Linear is the supported production tracker for this scaffold. Non-Linear adapters such as GitHub Issues, Jira, or Shortcut are deferred for Linear-only deployments and should be revisited only when there is a concrete deployment target and test profile.
...
`tracker.handoff_state` is parsed as a scalar string. For example, use `handoff_state: "Human Review"` rather than a YAML list. The target state must exist in the configured Linear team's workflow, and it should not be included in `active_states`.
